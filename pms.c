...
static const uint_least8_t CMD_REQUEST_READ [7] = {0x42, 0x4D, 0xE2, 0x00, 0x00, 0x01, 0x71};
static const uint_least8_t CMD_MODE_ACTIVE [7] = {0x42, 0x4D, 0xE1, 0x00, 0X01, 0x01, 0x71};
static const uint_least8_t CMD_MODE_PASSIVE [7] = {0x42, 0x4D, 0xE1, 0x00, 0x00, 0x01, 0x70};
...
static const uart_config_t UART_CONFIG = { //config UART from documentation PMS
    .baud_rate = 9600,
...
static pms_error_t pms_uart_init(void)
{
    ...
    {   
        if(uart_param_config(PMS_UART_NUM , &UART_CONFIG) !=0 ||
            uart_set_pin(PMS_UART_NUM , PMS_TX_GPIO, PMS_RX_GPIO, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE) !=0 ||
            uart_driver_install(PMS_UART_NUM , PMS_UART_BUFFER_RX_SIZE, PMS_UART_BUFFER_TX_SIZE, 2, &uart_queue, 0) !=0)
        {
...
pms_error_t pms_set_workmode(const pms_workmode_t workmode) //send command set mode active/passive
{
    ...
    {
        if(sizeof(CMD_MODE_ACTIVE) != uart_tx_chars(PMS_UART_NUM, &CMD_MODE_ACTIVE, sizeof(CMD_MODE_ACTIVE))) 
        {
    ...
    {
        if(sizeof(CMD_MODE_PASSIVE) != uart_tx_chars(PMS_UART_NUM, &CMD_MODE_PASSIVE, sizeof(CMD_MODE_PASSIVE))) 
        {
...
pms_error_t pms_request_read(pms_measurement_t *dst) //send command "get measurement results" and read data -> function pms_read_from_buffer
{
    ...
    if(sizeof(CMD_REQUEST_READ) != uart_tx_chars(PMS_UART_NUM, &CMD_REQUEST_READ, sizeof(CMD_REQUEST_READ)))
    {
