...
// int type state becouse gpio_get_level return int -> == the same type is faster than other type
...
// wait for response DHT
    ...
    get_state_time_us(0, 260, &usTimeState);
    if(usTimeState < 0)
    {
    ...
    get_state_time_us(1, 260, &usTimeState);
    if(usTimeState < 0)
    {
