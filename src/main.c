...
#define TIME_SLEEP_MS           320000 //real + DELAY_START_PMS
...
struct __attribute__((__packed__)) PayloadMeasurement  {
    ...
    uint8_t     type:2; 
    uint16_t    temperature:12; //range -400 : 1250 => 0  : 2048 => 1signed bit + 11bits
...
void measure_dht(dht_measurement_t *dht_value_1h)
{
    ...
    for(uint8_t i=0; i<MAX_NUM_TRY_MEASUREMENT; ++i){
        ...
        {
            if(++offset_tmp>(MAX_NUM_MEASUREMENT-1))break;
        } 
...
void make_adv_data(const dht_measurement_t *dht_value_1h, const pms_measurement_t *pms_value_1h, uint8_t esp_temp, uint8_t type)
{
    ...
    const struct PayloadMeasurement payload={
        .type=type,
        .temperature=dht_value_1h->temperature | ((dht_value_1h->temperature&0x8000)>>4),//temperature and set signed bit on 12bit
        ...
        .um[0]=pms_value_1h->num.um3,
        .um[1]=pms_value_1h->num.um5,
        .um[2]=pms_value_1h->num.um10,
        .um[3]=pms_value_1h->num.um25,
        .um[4]=pms_value_1h->num.um50,
        .um[5]=pms_value_1h->num.um100,
